...
;(function (global, undefined) {
  var _camera, _scene, _renderer, _mesh
  var _cameraOrtho, _sceneOrtho, timeoutEvent
  var _fov = 75
  var _pRadius = 1000
  var _raycaster
  var _container
  ...
  tpanorama.prototype = {
    ...
    render: function render(opt) {
      ...
      _container.addEventListener('touchend', onDocumentTouchEnd.bind(this), false)
    ...
    reRender: function reRender(opt) {
      // 重置label和场景数组
  ...
  function makePanorama(pRadius, widthSegments, heightSegments, u) {
    try {
      _mesh = new THREE.Mesh(
        new THREE.SphereGeometry(pRadius, widthSegments, heightSegments), //几何形状--球形
        new THREE.MeshBasicMaterial({ map: THREE.ImageUtils.loadTexture(u) }) //材料--贴图材料
      )
      _mesh.scale.x = -1
      _scene.add(_mesh)
      _scene.add(_mesh)
    } catch (error) {
      console.log('makePanorama ===>', error)
    }
  }
  ...
  function initRenderer() {
    ...
    _container.appendChild(_renderer.domElement) //渲染器挂载 即canvas挂载dom
  }
  ...
  function onDocumentTouchEnd() {
    ...
    if (timeoutEvent !== 0) {
      // 点击action
      _mouse.x = (_onPointerDownPointerX / window.innerWidth) * 2 - 1
      _mouse.y = -(_onPointerDownPointerY / window.innerHeight) * 2 + 1
  ...
  function initLable(lables, sprite) {
    if (sprite == 'label') {
      ...
      for (var i = 0; i < lables.length; i++) {
        _sprites.push(createSprite(lables[i].position, lables[i].logoUrl, lables[i].text, lables[i].size))
      }
    }
  }
  ...
  // 创建icon
  function createSprite(position, url, name, size = { w: 32, h: 32 }) {
    ...
    // 贴图的缩放
    sp1.sprite.scale.set(size, size, 1.0)
    ...
    if (typeof _sprites != 'undefined') {
      for (var i = 0; i < _sprites.length; i++) {
        ...
        if (test.x > -1 && test.x < 1 && test.y > -1 && test.y < 1 && test.z > -1 && test.z < 1) {
          _sprites[i].sprite.scale.set(_sprites[i].size.w, _sprites[i].size.h, 1)
  ...
  // 鼠标悬停在canvas上就会调用  ====>  TODO
  function calLonLat(e) {
    // console.log('_setContainer.style.height ===>',_setContainer.style.height);
