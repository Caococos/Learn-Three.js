...
;(function (global, undefined) {
  ...
  function initCamera() {
    try {
      ...
      _controls = new THREE.OrbitControls(_camera, render.domElement) //创建控件对象
  ...
  function makePanorama(pRadius, widthSegments, heightSegments, u) {
    try {
      var mesh = new THREE.Mesh(
        new THREE.SphereGeometry(pRadius, widthSegments, heightSegments),
        new THREE.MeshBasicMaterial({ map: THREE.ImageUtils.loadTexture(u) })
      )
      mesh.scale.x = -1
      _scene.add(mesh)
    } catch (error) {
